...
## Prerequisites
- `data/purchase-listings.json` must exist and have internet data (run `/purchase:search-listings` and `/purchase:check-internet` first)

## Inputs
None — reads from `data/purchase-listings.json` and `data/screenshots/`

## Workflow

1. Read `data/purchase-listings.json`. Validate that listings exist and most have internet data.
...
5. **Fetch Google Maps images** for each listing (requires API key from `data/.env`):
   ...
   - Download via `curl` to `data/screenshots/{id}-map.jpg` and `{id}-streetview.jpg`
...
6. **Build HTML report** (primary format):
   ...
   **Filename**: `data/portland-purchase-report-YYYYMMDD-HHMM.html`

7. Tell user: "Report generated with X properties."

## Expected Output
- `data/portland-purchase-report-YYYYMMDD-HHMM.html` (timestamped)
