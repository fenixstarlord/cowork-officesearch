...
## Prerequisites
- `data/listings.json` must exist and have internet data (run `/rental:search-listings` and `/rental:check-internet` first)

## Inputs
None — reads from `data/listings.json` and `data/screenshots/`

## Workflow

1. Read `data/listings.json`. Validate that listings exist and most have internet data.
...
5. **Fetch Google Maps images** for each listing (requires API key from `data/.env`):
   ...
   - Download via `curl` to `data/screenshots/{id}-map.jpg` and `data/screenshots/{id}-streetview.jpg`
...
6. **Build HTML report** (primary format) using the `Write` tool:
   ...
   **Filename**: `data/portland-apartment-report-YYYYMMDD-HHMM.html` (timestamped)

7. **Alternative: Build PDF** if user requests PDF format, using the `anthropic-skills:pdf` skill. PDF also includes photo galleries. Save to `data/portland-apartment-report.pdf`.

8. Tell user: "Report generated at data/portland-apartment-report-YYYYMMDD-HHMM.html with X listings."

## Expected Output
- `data/portland-apartment-report-YYYYMMDD-HHMM.html` (primary, timestamped)
- `data/portland-apartment-report.pdf` (alternative, if requested)
